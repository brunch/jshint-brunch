...
module.exports = JSHintLinter = (function() {
  ...
  JSHintLinter.prototype.lint = function(data, path, callback) {
    ...
    if (success) {
      ...
      errors = jshint.errors.filter(function(elem) {
        return elem != null;
      });
      if (this.reporter) {
        results = [];
        ...
        msg = this.warnOnly ? 'warn: (JSHint)' : 'err: (JSHint)';
      } else {
        errorMsg = [];
        fn1 = (function(_this) {
          return function(error) {
            var ref;
            if (((ref = error.evidence) != null ? ref.length : void 0) < 160) {
              return errorMsg.push((pad(error.line.toString(), 7)) + " | " + (chalk.gray(error.evidence)) + "\n" + (pad("^", 10 + error.character)) + " " + (chalk.bold(error.reason)));
            } else {
              return errorMsg.push((pad(error.line.toString(), 7)) + " | col: " + error.character + ": " + (chalk.bold(error.reason)));
            }
          };
        })(this);
